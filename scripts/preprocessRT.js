...
var minRT = process.argv[3] || 2865732.500000;  //worst case RT start
var maxRT = process.argv[4] || 2865834.500000;  //wrost caset RT end
...
function groupLpByRt() {
    ...
    csv.read({
        filepath  : DATASET  + '/ross-stats-rt-lps.csv',
        skip: 1,
        delimiter : ",",
        ...
        oncomplete: function() {
            ...
            fs.writeFile('./data/ross-stats-rt-lps.json', JSON.stringify(result), function(err) {
...
function groupKpByRt() {
    ...
    csv.read({
        filepath  : DATASET + '/ross-stats-rt-kps.csv',
        skip: 1,
        delimiter : ",",
        // bufferSize: 8 * 1024 * 1024,
        onload    : processData(headers, dataTypes, features),
        oncomplete: function() {
            ...
            fs.writeFile('./ross-stats-rt-kps.json', JSON.stringify(result), function(err) {
                if(err) {return console.log(err);}
                console.log("The file was saved!");
                groupLpByRt();
            });
...
function groupPeByRt() {
    ...
    csv.read({
        ...
        oncomplete: function() {
            ...
            fs.writeFile('./ross-stats-rt-pes.json', JSON.stringify(result), function(err) {
                if(err) {return console.log(err);}
                console.log("The file was saved!");
                groupKpByRt();
            });
